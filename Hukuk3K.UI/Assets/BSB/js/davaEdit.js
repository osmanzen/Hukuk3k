document.querySelector('#davaguncelle').addEventListener('submit', function (e) {
    var form = this;
    e.preventDefault();
...
var drpDavaBaslik = $("#drpDavaBaslik");
...
var basliklar = function () {
    drpDavaBaslik.empty().selectpicker('refresh');
    $.ajax({
        url: "/Admin/Dava/BaslikGetir",
        type: "POST",
        success: function (response) {
            $.each(response, function (index, baslik) {
                drpDavaBaslik.append("<option value='" + baslik.DavaId + "' >" + baslik.BaslikAdi + "</option>");
            });
            drpDavaBaslik.selectpicker('refresh');
        },
        failure: function (response) {
            alert("Eklenemedi");
        },
...
davaBaslikEkle.click(function () {
    swal({
        title: "Davaya Başlık Ekle",
        type: "input",
        showCancelButton: true,
        closeOnConfirm: false,
        animation: "slide-from-bottom",
        inputPlaceholder: "Dava Başlığı",
        cancelButtonText: 'İptal',
        confirmButtonText: "Kaydet"
    }, function (inputValue) {
        if (inputValue === false) return false;
        if (inputValue === "") {
            ...
            $.ajax({
                url: "/Admin/Dava/BaslikEkle",
                data: { baslikAdi: inputValue },
                dataType: "JSON",
                type: "POST",
                success: function (baslik) {
                    basliklar();
                    swal("Başarılı!", baslik.DavaBaslikAdi + " Başlığı Davaya Eklendi.", "success");
                }
            });
...
davaBaslikDuzenle.click(function () {
    swal({
        title: "Başlık Adı Değiştir",
        type: "input",
        showCancelButton: true,
        closeOnConfirm: false,
        animation: "slide-from-bottom",
        inputPlaceholder: "Başlık Adı",
        cancelButtonText: 'İptal',
        confirmButtonText: "Güncelle"
    }, function (inputValue) {
        if (inputValue === false) return false;
        if (inputValue === "") {
            ...
            $.ajax({
                url: "/Admin/Dava/BaslikGuncelle",
                data: { baslikAdi: inputValue, baslikId: drpDavaBaslik.val() },
                dataType: "JSON",
                type: "POST",
                success: function (baslik) {
                    basliklar();
                    swal("Başarılı!", baslik.DavaBaslikAdi + " Başlık Güncellendi.", "success");
                }
            });
        }
    });
...
davaBaslikSil.click(function () {
    swal({
        title: "Seçilen Başlığı Silmeyi Onaylıyor musunuz?",
        text: "",
        type: "warning",
        showCancelButton: true,
        cancelButtonText: 'İptal',
        ...
        function () {
            $.ajax({
                url: "/Admin/Dava/BaslikSil",
                data: { baslikId: drpDavaBaslik.val() },
                dataType: "JSON",
                type: "POST",
                success: function (baslik) {
                    basliklar();
                    swal("Başarılı!", " Baslik Silindi.", "success");
                }
            });
        });
});
...
drpDavaBaslik.change(function () {
    davaPdfleriGetir();
});

function davaPdfleriGetir() {
    $.ajax({
        type: "POST",
        url: "/Dava/DavaPdfGetir/" + drpDavaBaslik.val(),
        contentType: "application/json; charset=utf-8",
        dataType: "html",
        success: function (response) {
            var dropdown = $('#davaDosyaDiv');
            dropdown.empty();
...
$(document).on("click", ".dosyaAdiDegistir", function () {
    var dosyaId = $(this).attr("data-id");

    swal({
        title: "Yeni Dosya Adı Giriniz",
        type: "input",
        showCancelButton: true,
        closeOnConfirm: false,
        animation: "slide-from-bottom",
        ...
        if (inputValue === "") {
            ...
            $.ajax({
                url: "/Admin/Dava/DosyaAdiDegistir",
                data: { davaDosyaId: dosyaId, yeniAd: inputValue },
                dataType: "JSON",
                type: "POST",
                success: function (baslik) {
                    swal("Başarılı!", " Dava Adı Değiştirildi.", "success");
                    davaPdfleriGetir();
                }
            });
        }
...
$(document).on("click", ".davaDosyaSil", function () {
    var dosyaId = $(this).attr("data-id");

    swal({
        title: "Dosya Silmeyi Onaylıyor musunuz?",
        text: "",
        type: "warning",
        showCancelButton: true,
        cancelButtonText: 'İptal',
        ...
        function () {
            $.ajax({
                url: "/Admin/Dava/DavaDosyaSil",
                data: { davaDosyaId: dosyaId },
                dataType: "JSON",
                type: "POST",
                success: function (baslik) {
                    swal("Başarılı!", " Dava Silindi.", "success");
                    davaPdfleriGetir();
                }
            });
        });
